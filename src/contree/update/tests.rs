use crate::{
    contree::{Albedo, Contree, ContreeEntry, BOX_NODE_CHILDREN_COUNT},
...
fn test_simple_insert_and_get_where_dim_is_1() {
    ...
    assert!(tree.get(&V3c::new(1, 1, 1)) == ContreeEntry::Empty);
...
fn test_insert_empty() {
    let mut tree: Contree = Contree::new(4, 1).ok().unwrap();
    tree.auto_simplify = false;
    tree.insert(&V3c::new(0, 0, 0), ContreeEntry::Empty).ok();
    assert!(tree.get(&V3c::new(0, 0, 0)) == ContreeEntry::Empty);
}
...
fn test_complex_insert_and_get() {
    ...
    assert!(
        hit == ContreeEntry::Empty,
        "Hit mismatch at {:?}: {:?} <> {:?}",
        (0, 0, 0),
        hit,
        ContreeEntry::<u32>::Empty
    );
...
fn test_case_simplified_insert_separated_by_clear_where_dim_is_1() {
    ...
    assert_eq!(tree.get(&V3c::new(3, 3, 3)), ContreeEntry::Empty);
...
fn test_update_data() {
    ...
    tree.update(&V3c::new(0, 0, 0), ContreeEntry::Informative(&4))
        .ok();
    let hit = tree.get(&V3c::new(0, 0, 0));
...
fn test_update_empty() {
    ...
    tree.update(&V3c::new(0, 0, 0), ContreeEntry::Empty).ok();
    assert!(tree.get(&V3c::new(0, 0, 0)) == (&red, &3).into());
}
...
fn test_uniform_solid_leaf_separated_by_clear_where_dim_is_1() {
    ...
    assert_eq!(tree.get(&V3c::unit(0)), ContreeEntry::Empty);
...
fn test_uniform_solid_leaf_separated_by_clear_where_dim_is_4() {
    ...
    tree.clear(&V3c::unit(0)).ok().unwrap();
    assert!(tree.get(&V3c::unit(0)) == ContreeEntry::Empty);
...
fn test_simple_clear_where_dim_is_1() {
    ...
    assert_eq!(tree.get(&V3c::new(0, 0, 1)), ContreeEntry::Empty);
    assert_eq!(tree.get(&V3c::new(1, 1, 1)), ContreeEntry::Empty);
}
...
fn test_double_clear() {
    ...
    assert!(item_at_001 == ContreeEntry::Empty);
}
...
fn test_simplifyable_clear() {
    ...
    assert!(item_at_000 == ContreeEntry::Empty);
    for x in 1..SIZE {
        for y in 1..SIZE {
            for z in 1..SIZE {
...
fn test_clear_to_nothing() {
    let albedo: Albedo = 0xFFAAEEFF.into();
    let entry = ContreeEntry::Visual(&albedo);
    let mut tree: Contree = Contree::new(4, 1).ok().unwrap();
    ...
    assert_eq!(tree.get(&V3c::new(0, 0, 0)), ContreeEntry::Empty);
    assert_eq!(tree.get(&V3c::new(0, 0, 1)), ContreeEntry::Empty);
    assert_eq!(tree.get(&V3c::new(0, 1, 0)), ContreeEntry::Empty);
    assert_eq!(tree.get(&V3c::new(0, 1, 1)), ContreeEntry::Empty);
    assert_eq!(tree.get(&V3c::new(1, 0, 0)), ContreeEntry::Empty);
    assert_eq!(tree.get(&V3c::new(1, 0, 1)), ContreeEntry::Empty);
    assert_eq!(tree.get(&V3c::new(1, 1, 0)), ContreeEntry::Empty);
    assert_eq!(tree.get(&V3c::new(1, 1, 1)), ContreeEntry::Empty);
}
...
fn test_clear_at_lod_with_unaligned_position_where_dim_is_1() {
    ...
    assert!(tree.get(&V3c::new(1, 1, 1)) == ContreeEntry::Empty);
    assert!(tree.get(&V3c::new(1, 1, 2)) == ContreeEntry::Empty);
    assert!(tree.get(&V3c::new(1, 2, 1)) == ContreeEntry::Empty);
    assert!(tree.get(&V3c::new(1, 2, 2)) == ContreeEntry::Empty);
    assert!(tree.get(&V3c::new(2, 1, 1)) == ContreeEntry::Empty);
    assert!(tree.get(&V3c::new(2, 1, 2)) == ContreeEntry::Empty);
    assert!(tree.get(&V3c::new(2, 2, 1)) == ContreeEntry::Empty);
    assert!(tree.get(&V3c::new(2, 2, 2)) == ContreeEntry::Empty);
...
fn test_overwrite_whole_nodes_where_dim_is_4() {
    ...
    for x in 0..8 {
        for y in 0..8 {
            for z in 0..8 {
                let hit = tree.get(&V3c::new(x, y, z));
                assert_ne!(hit, ContreeEntry::Empty);
                match hit {
                    ContreeEntry::Visual(hit) => {
                        if *hit == red {
                            hits_red += 1
                        }
                        if *hit == blue {
